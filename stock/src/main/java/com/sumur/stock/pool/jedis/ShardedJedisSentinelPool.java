...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	public ShardedJedisSentinelPool(String masterNames, String sentinels, final GenericObjectPoolConfig poolConfig, final int timeout) {
		this(masterNames, sentinels, poolConfig, timeout, null);
	}

	public ShardedJedisSentinelPool(String masterNames, String sentinels, final GenericObjectPoolConfig poolConfig, int timeout, final String password) {
	...
	private List<HostAndPort> getAllMasterAddrFromSentinel(Set<String> sentinels, final List<String> masters) {
		...
		for (String masterName : masters) {
			...
			while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
				...
				if (master == null) {
					try {
						logger.error("All sentinels down, cannot determine where is " + masterName + " master is running... sleeping 1000ms, Will try again.");
						Thread.sleep(1000L);
		...
		if (masters.size() == shardMasters.size()) {
			...
			throw new JedisConnectionException("Not all shards master can be accessed. Except " + masters.size() + " masters, but " + shardMasters.size() + " masters can be accessed");
		}
								...
								if (index >= 0) {
									try {
										...
										if (!equalsList(newHostMasters, currentHostMasters)) {
											...
											logger.warn("SentinelSwitched M/S of [" + message + "](from > to), completed reInitPool.");
										}
									} finally {
										reInitLock.unlock();
									}
									...
									logger.warn("Ignoring message on +switch-master for master name " + switchMasterMsg[0] + ", our monitor master name are [" + sb + "]");
								}
							} else {
